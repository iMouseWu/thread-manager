...
public class ExecutorTaskManagerImpl implements ExecutorTaskManager {
	...
	// LoggerFactory.getLogger(ExecutorTaskManagerImpl.class);
	...
	private boolean needContinueAndInit(ExecutorTask executorTask) {
		...
		if (threadDO.getThreadStatus() == ThreadStatus.EXCEPTION) {
			needContinue = executorDao.updateExcutorTaskStatusAndRetryTime(threadDO.getTaskId(), threadDO.getIp(),
					ThreadStatus.EXCEPTION, ThreadStatus.INIT);
		}
		return needContinue;
	}
	...
	private class Work implements Runnable {
		...
		public void run() {
			...
			try {
				...
				System.out.println(threadDO.getTaskName() + " throw Exception the times is " + threadDO.getRetryTime());
		...
		private void doAfterException(ThreadDO threadDO) {
			...
			executorDao.updateExcutorTaskStatusAndExecuteTime(taskId, ip, ThreadStatus.DOING, ThreadStatus.EXCEPTION,
					nextExecuteTime);
			ExceptionRetryTask exceptionRetryTask = new ExceptionRetryTask(threadDO);
			exceptionTaskResolve.add(exceptionRetryTask);
		}
	}
}
